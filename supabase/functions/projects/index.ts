...
import {
  ...
  getProjectPartners, addProjectPartner, updatePartnerRole, removeProjectPartner,
...
// GET /functions/v1/projects/feed
async function feed(req: Request): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;
  if (user.role !== "investor") return json({ error: "Only investors have a project feed" }, 403);

  return json(await getProjectFeed(user.id));
}

// POST /functions/v1/projects/swipe  { projectId, direction }
async function swipe(req: Request): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;
  if (user.role !== "investor") return json({ error: "Only investors can swipe on projects" }, 403);

  const body = await req.json().catch(() => ({}));
  const { projectId, direction } = body as { projectId?: string | number; direction?: string };
  if (!projectId || !["like", "pass"].includes(direction ?? "")) {
    return json({ error: "projectId and direction (like|pass) are required" }, 400);
  }

  const result = await swipeProject(user.id, projectId, direction!);
  if ((result as { error?: string }).error) return json(result, 400);
  return json(result);
}

// GET /functions/v1/projects/matches
async function matches(req: Request): Promise<Response> {
  ...
  return json(await getProjectMatches(user.id, user.role as string));
}

// GET /functions/v1/projects/joined
async function joined(req: Request): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;

  return json(await getJoinedProjects(user.id));
}

// GET /functions/v1/projects/owner/:userId
async function byOwner(req: Request, params: Record<string, string>): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);

  return json(await getProjectsByOwner(params.userId));
}

// GET /functions/v1/projects/:id/partners
async function listPartners(req: Request, params: Record<string, string>): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);

  return json(await getProjectPartners(params.id));
}

// POST /functions/v1/projects/:id/partners  { partnerUserId, role }
async function addPartner(req: Request, params: Record<string, string>): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;
  ...
  const body = await req.json().catch(() => ({}));
  const { partnerUserId, role } = body as { partnerUserId?: string; role?: string };
  if (!partnerUserId) return json({ error: "partnerUserId is required" }, 400);

  const result = await addProjectPartner(params.id, user.id, partnerUserId, role);
  if ((result as { error?: string }).error) return json(result, 403);
  return json(result, 201);
}

// PUT /functions/v1/projects/:id/partners/:userId/role  { role }
async function patchPartnerRole(req: Request, params: Record<string, string>): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;
  ...
  const result = await updatePartnerRole(params.id, user.id, params.userId, role ?? "member");
  if ((result as { error?: string }).error) return json(result, 403);
  return json(result);
}

// DELETE /functions/v1/projects/:id/partners/:userId
async function removePartner(req: Request, params: Record<string, string>): Promise<Response> {
  const user = await authenticate(req);
  if (!user) return json({ error: "Unauthorized" }, 401);
  const verifyErr = requireVerified(user);
  if (verifyErr) return verifyErr;

  const result = await removeProjectPartner(params.id, user.id, params.userId);
  if ((result as { error?: string }).error) return json(result, 403);
  return json(result);
}
...
serveFunction(FN, [
  ...
  route(FN, "GET", "/joined", joined),
  route(FN, "GET", "/owner/:userId", byOwner),
  ...
  route(FN, "GET", "/:id/partners", listPartners),
  route(FN, "POST", "/:id/partners", addPartner),
  route(FN, "PUT", "/:id/partners/:userId/role", patchPartnerRole),
  route(FN, "DELETE", "/:id/partners/:userId", removePartner),
